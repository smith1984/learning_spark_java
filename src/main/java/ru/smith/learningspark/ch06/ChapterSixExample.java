...
public class ChapterSixExample {
  ...
  public static void main(String[] args) throws Exception {
    ...
    rdd.foreach((VoidFunction<String>) line -> {
    ...
    JavaRDD<String> validCallSigns = callSigns.filter(
            (Function<String, Boolean>) callSign -> {
    ...
    JavaPairRDD<String, Integer> contactCounts = validCallSigns.mapToPair(
            (PairFunction<String, String, Integer>) callSign -> new Tuple2(callSign, 1)).reduceByKey((Function2<Integer, Integer, Integer>) Integer::sum);
    ...
    JavaPairRDD<String, Integer> countryContactCounts = contactCounts.mapToPair(
            (PairFunction<Tuple2<String, Integer>, String, Integer>) callSignCount -> {
    ...
    JavaPairRDD<String, CallLog[]> contactsContactLists = validCallSigns.mapPartitionsToPair(
            (PairFlatMapFunction<Iterator<String>, String, CallLog[]>) input -> {
    ...
    JavaRDD<String> pipeInputs = contactsContactLists.values().map(new VerifyCallLogs()).flatMap(
            (FlatMapFunction<CallLog[], String>) calls -> {
    ...
    JavaDoubleRDD distanceDoubles = distances.mapToDouble((DoubleFunction<String>) value -> Double.parseDouble(value));
    final StatCounter stats = distanceDoubles.stats();
    final Double stddev = stats.stdev();
    final Double mean = stats.mean();
    JavaDoubleRDD reasonableDistances =
      distanceDoubles.filter((Function<Double, Boolean>) x -> (Math.abs(x-mean) < 3 * stddev));
